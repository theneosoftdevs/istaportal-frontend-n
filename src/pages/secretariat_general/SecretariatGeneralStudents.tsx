...
export function SecretariatGeneralStudents() {
  ...
  const { data, loading } = usePageData((d) => {
    // Use enrichStudent to ensure promotion/faculty/academic year are resolved from histories or nested objects
    const students: StudentRow[] = d.students.map(
      (s) => enrichStudent(d, s) as StudentRow,
    );
    ...
    <>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Select value={promotion} onValueChange={setPromotion}>
          ...
          <SelectContent>
            <SelectItem value="all">
              {i18n.apparitorat.all_promotions}
            </SelectItem>
            {data?.promotions
